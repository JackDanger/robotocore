...
class TestDotnetCompileEnv:
    ...
    def test_detect_tfm_passes_invariant_env_to_subprocess(self):
        ...
        dotnet_mod.invalidate_caches()  # the probe result is cached across calls
        mock_proc = MagicMock()
        ...
        assert passed_env is not None, "the dotnet probe did not pass env= to subprocess.run"
        assert passed_env.get("DOTNET_SYSTEM_GLOBALIZATION_INVARIANT") == "1", (
            "the dotnet probe is missing DOTNET_SYSTEM_GLOBALIZATION_INVARIANT; "
